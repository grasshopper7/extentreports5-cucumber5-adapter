...
public class ExtentService implements Serializable {
	...
	private static class ExtentReportsLoader {
		...
		private static void createViaSystem() {
			/*
			 * if ("true".equals(System.getProperty(INIT_AVENT_KEY))) initAvent(null);
			 * 
			 * if ("true".equals(System.getProperty(INIT_BDD_KEY))) initBdd(null);
			 * 
			 * if ("true".equals(System.getProperty(INIT_CARDS_KEY))) initCards(null);
			 * 
			 * if ("true".equals(System.getProperty(INIT_EMAIL_KEY))) initEmail(null);
			 * 
			 * if ("true".equals(System.getProperty(INIT_HTML_KEY))) initHtml(null);
			 */

			/*
			 * if ("true".equals(System.getProperty(INIT_KLOV_KEY))) initKlov(null);
			 */

			/*
			 * if ("true".equals(System.getProperty(INIT_LOGGER_KEY))) initLogger(null);
			 */
			...
			/*
			 * if ("true".equals(System.getProperty(INIT_TABULAR_KEY))) initTabular(null);
			 */
		...
		/*
		 * private static void initAvent(Properties properties) { String out =
		 * getOutputPath(properties, OUT_AVENT_KEY); ExtentAventReporter avent = new
		 * ExtentAventReporter(out); attach(avent, properties, CONFIG_AVENT_KEY); }
		 */
		...
		/*
		 * private static void initBdd(Properties properties) { String out =
		 * getOutputPath(properties, OUT_BDD_KEY); ExtentBDDReporter bdd = new
		 * ExtentBDDReporter(out); attach(bdd, properties, CONFIG_BDD_KEY); }
		 * 
		 * private static void initCards(Properties properties) { String out =
		 * getOutputPath(properties, OUT_CARDS_KEY); ExtentCardsReporter cards = new
		 * ExtentCardsReporter(out); attach(cards, properties, CONFIG_CARDS_KEY); }
		 * 
		 * private static void initEmail(Properties properties) { String out =
		 * getOutputPath(properties, OUT_EMAIL_KEY); ExtentEmailReporter email = new
		 * ExtentEmailReporter(out); attach(email, properties, CONFIG_EMAIL_KEY); }
		 * 
		 * private static void initHtml(Properties properties) { String out =
		 * getOutputPath(properties, OUT_HTML_KEY); ExtentHtmlReporter html = new
		 * ExtentHtmlReporter(out); attach(html, properties, CONFIG_HTML_KEY); }
		 */

		/*
		 * private static void initKlov(Properties properties) { ExtentKlovReporter klov
		 * = new ExtentKlovReporter("Default"); String configPath = properties == null ?
		 * System.getProperty(CONFIG_KLOV_KEY) :
		 * String.valueOf(properties.get(CONFIG_KLOV_KEY)); if (configPath != null &&
		 * !configPath.isEmpty()) try { klov.loadInitializationParams(configPath);
		 * INSTANCE.attachReporter(klov); } catch (FileNotFoundException e) {
		 * e.printStackTrace(); } }
		 */

		/*
		 * private static void initLogger(Properties properties) { String out =
		 * getOutputPath(properties, OUT_LOGGER_KEY); ExtentLoggerReporter logger = new
		 * ExtentLoggerReporter(out); attach(logger, properties, CONFIG_LOGGER_KEY); }
		 */
		 ...
		 * getOutputPath(properties, OUT_TABULAR_KEY); ExtentTabularReporter tabular =
		 * new ExtentTabularReporter(out); attach(tabular, properties,
		 * CONFIG_TABULAR_KEY); }
		 */
